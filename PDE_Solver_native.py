...
class Params:
    ...
    def print_params(self):
      print(self.gamma, self.alpha, self.beta, self.zeta, self.kappa, self.rho0)
...
class PheromoneEnzymeSolver:
    ...
    def __init__(self, params: Params, grid: Grid, show_plots=False):
        self.p = params
        self.g = grid
    ...
    def solve(self, max_iter=200000, tol=5e-8, print_every=10000):
        ...
        for n in range(max_iter):
            ...
            dPdt    = lap_P + self.S_P
            rxn     = 4*np.pi*p.zeta * ((self.Q - psi)*(P - psi) - p.kappa*psi)
            dpsi_dt = lap_psi + rxn
    ...
    def plot_axis(self, savepath=None):
        j_axis = 0  # theta closest to 0 (toward source)

        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(self.r, self.phi[:, j_axis], 'r-',  lw=2, label='φ (enzyme)')
        ax.plot(self.r, self.chi[:, j_axis], 'b-',  lw=2, label='χ (free pheromone)')
        ax.plot(self.r, self.psi[:, j_axis], 'g-',  lw=2, label='ψ (complex)')
        ax.plot(self.r, self.Q[:, j_axis],   'r--', lw=1, alpha=0.4, label='Q = φ+ψ')
        ax.plot(self.r, self.P[:, j_axis],   'b--', lw=1, alpha=0.4, label='P = χ+ψ')
...
if __name__ == "__main__":
    ...
    ax = plt.gca()
    fig = plt.gcf()
    img = solver.plot_metric_heatmap(param1="zeta", param2="beta", param1_max=0.5, num_cells_per_side=5, ax=ax)
